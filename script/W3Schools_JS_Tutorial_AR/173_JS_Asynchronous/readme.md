...
from top to bottom
and left to right.
Control flow statements let you
change that order
, based on conditions, loops or keywords.
Default Flow
Default flow executes code
sequentially
...
refers to how JavaScript handles tasks that take time to
complete, like reading files, or waiting for user input,
without blocking the execution of other code.
To prevent blocking, JavaScript can use
asynchronous programming
.
This allows certain operations to run in the background,
and their results are handled later, when they are ready.
...
and is intended to be executed at a later point in time, typically
when a specific event occurs or an asynchronous operation completes.
...
Async/Await
is a modern, cleaner way to handle asynchronous code.
...
Description
Synchronus
Executes line by line, blocking the next task.
Asynchonus
Allows other code to run while waiting for tasks to finish.
Events
Stores callback function waiting to be executed.
Promises
Tools to handle asynchronous operations cleanly.
Async/Await
A modern, cleaner way to handle asynchronous code.
★
+1
